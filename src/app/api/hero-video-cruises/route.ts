...
export async function POST(request: NextRequest) {
  try {
    ...
    const newFilePath = join(cruisesDir, oldFileName);
    ...
    console.error("Ошибка загрузки hero видео круизов:", error);
    return NextResponse.json(
...
export async function GET() {
  try {
    ...
    const cruisesDir = join(process.cwd(), "public", "cruises");
    const files: string[] = await readdir(cruisesDir).catch(() => []);
    ...
    console.error("Ошибка получения информации о hero видео круизов:", error);
    return NextResponse.json(
